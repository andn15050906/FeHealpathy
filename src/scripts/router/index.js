...
import {
  getUserBearerToken,
  getRole,
} from "@/scripts/api/services/authService";
...
const router = createRouter({
  ...
  routes: [
    ...
    {
      path: "/roadmap/:roadmapId/phase/:phaseId",
      name: "phase-detail",
      component: StepDetail,
      props: true,
    },
...
const isAuthorized = (to) => {
  ...
  if (!getUserBearerToken()) return false;

  let role = getRole();
  if (
    to.matched.some(
      (record) =>
        record.meta.requiresAdvisorOrAdmin &&
        !["Advisor", "Admin"].includes(role)
    )
  )
    return false;
  if (
    to.matched.some((record) => record.meta.requiresAdvisor) &&
    role != "Advisor"
  )
    return false;
