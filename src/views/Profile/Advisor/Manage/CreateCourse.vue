  ...
  methods: {
    ...
    addLecture() {
      this.course.lectures.push({
        ...
        contentSummary: "",
        isPreviewable: false,
        index: this.course.lectures.length + 1,
        lectureType: "Video",
        metaData: "",
        courseId: null,
        medias: []
    ...
    async saveCourse() {
      ...
      try {
        ...
        formData.append("Intro", this.course.intro || "");
        formData.append("Description", this.course.description || "");
        formData.append("Status", this.course.status || 0);
        formData.append("Price", this.course.price || 0);
        formData.append("Discount", this.course.discount || 0);
        
        // Handle discount expiry - only send if there's a discount
        if (this.course.discount > 0) {
          if (this.course.discountExpiry) {
            formData.append("DiscountExpiry", this.course.discountExpiry);
          } else {
            // If discount > 0 but no expiry date, set to tomorrow
            const tomorrow = new Date();
            tomorrow.setDate(tomorrow.getDate() + 1);
            formData.append("DiscountExpiry", tomorrow.toISOString().split('T')[0]);
          }
        } else {
          // If no discount, don't send DiscountExpiry at all
          formData.append("DiscountExpiry", "");
        }
        
        formData.append("Level", this.course.level || 0);
        formData.append("AdvisorExpectedOutcome", this.course.advisorExpectedOutcome || "");
        formData.append("Outcomes", this.course.outcomes || "");
        formData.append("Requirements", this.course.requirements || "");
        formData.append("ExpectedCompletion", this.course.completionTime || 0);
        formData.append("LeafCategoryId", this.course.leafCategoryId);

        // Handle thumbnail
        if (this.course.thumb.file) {
          formData.append("Thumb.File", this.course.thumb.file);
          formData.append("Thumb.Title", this.course.thumb.title || this.course.title);
        ...
        this.course.lectures.forEach((lecture, index) => {
          ...
          formData.append(`Lectures[${index}].Index`, index + 1);
          formData.append(`Lectures[${index}].LectureType`, lecture.type || "Video");
          formData.append(`Lectures[${index}].MetaData`, lecture.metaData || "");
          ...
          if (lecture.medias && lecture.medias.length > 0) {
            lecture.medias.forEach((media, mediaIndex) => {
              if (media.file) {
                formData.append(`Lectures[${index}].Medias[${mediaIndex}].File`, media.file);
                formData.append(`Lectures[${index}].Medias[${mediaIndex}].Title`, media.title);
                formData.append(`Lectures[${index}].Medias[${mediaIndex}].Url`, media.url || "");
                formData.append(`Lectures[${index}].Medias[${mediaIndex}].Type`, media.type);
              }
            });
          } else {
            // If no media, send empty array
            formData.append(`Lectures[${index}].Medias`, "[]");
          }
        });
        ...
        // If course creation is successful, we can use lectureService for future updates
        if (response && response.id) {
          // Store courseId for future lecture operations
